...
{
  examples = [{name:"All Data",code: "[{$sort:{timeNow:-1}},{$limit:5000}]"}]
 console.log(examples)
    ...
    calcMoveTime = { $set : { moveTime : { 
        $subtract:["$gameTime","$moveTime"]}}}



    avgByPlayer = {$group:{_id:"$playerId", longest: {$max: "$moveTime"}, ave: {$avg:"$moveTime"},moves:{$sum:1}}}
    fastfirst= {$sort: {ave:1}}
    tidyUp = {$set: { playerId:"$_id"}}
  
  playerAnalysisPipeline = JSON.stringify([noLongPause,getLastMove,calcMoveTime,avgByPlayer,fastfirst,tidyUp],null,2)
  playerAnalysisPipeline = compactJSON(playerAnalysisPipeline);
  examples.push({name:"Average Move Speed",code : playerAnalysisPipeline })
...
{
  vueApp.aggregation = vueApp.selectedExample;
}
...
function runChart() {
  ...
  vueApp.chart = new Chart("myChart", {
    type: "scatter", data: { datasets: [{  pointRadius: 2, 
